...
**Promotion parked 2026-04-24.** Phase α is deployed and running in `agentopia-dev` (see §15 status block), but §14.5 criterion 4 (off-cluster backup) is blocked by [`agentopia-infra#168`](https://github.com/ai-agentopia/agentopia-infra/issues/168), which was placed **On Hold / Deferred** on the same date. The backup decision ([ADR-016](../../adrs/016-h3-off-cluster-backup-cloudflare-r2)) and infra groundwork are merged and resumable; no architectural work remains. This document stays Draft until #168 resumes and completes. Accepted interim risk: node/disk loss on the single k3s host = loss of observability data up to the §8 retention ceiling; the `langfuse` database on shared Postgres is unaffected (shared-system backup covers it).
...
3. **Off-cluster backup target is resolved.** [`agentopia-infra#168`](https://github.com/ai-agentopia/agentopia-infra/issues/168) must be closed — "backing up a cluster to itself is not a backup." **Status: On Hold 2026-04-24.** Ticket deferred; decision and groundwork landed ([ADR-016](../../adrs/016-h3-off-cluster-backup-cloudflare-r2), [infra#175](https://github.com/ai-agentopia/agentopia-infra/pull/175)) but activation is not scheduled. This criterion is consequently parked.
...
2. **Off-cluster backup target** ([`#168`](https://github.com/ai-agentopia/agentopia-infra/issues/168)). **Decision resolved, execution On Hold 2026-04-24.** [ADR-016](../../adrs/016-h3-off-cluster-backup-cloudflare-r2) selected Cloudflare R2; [infra#175](https://github.com/ai-agentopia/agentopia-infra/pull/175) merged the CronJob manifests + seed script. Activation (R2 bucket provisioning, CronJob apply+unsuspend, first restore rehearsal) is deferred pending product/ops re-prioritization. The `langfuse` Postgres database backup is covered by the shared system — that system's off-cluster backup posture remains to be confirmed as part of the activation round.
