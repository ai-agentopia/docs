...
4. **Observability is integrated, not built.** OpenTelemetry wire format + OpenInference semantic conventions; Langfuse (MIT core) as the backend per [ADR-015](../../adrs/015-h3-02-trace-backend-langfuse). Production design at [H3 Observability Production Design](./h3-observability-production-design).
...
- **Phase H3 — traces / evals / checkpoints.** Instrument gateway with OTEL + OpenInference; stand up Langfuse per ADR-015 against the production design at [h3-observability-production-design](./h3-observability-production-design); checkpoint policy matrix live for irreversible actions; unify workflow-, thread-, and tool-boundary approval semantics without forcing all approvals into one hop; one task class with eval coverage.
