...
| Human checkpoints | Temporal signals + control-plane policy matrix | Pause for approval at policy-bearing boundaries |
...
- **Phase H3 — traces / evals / checkpoints.** Instrument gateway with OTEL + OpenInference; stand up Phoenix or Langfuse; checkpoint policy matrix live for irreversible actions; one task class with eval coverage.
