...
Agentopia can support a production-quality Human-in-the-Loop / checkpoint system **without a greenfield runtime rewrite**, but the fit is not uniform across lanes. The deterministic lane already runs a production HITL pattern — Temporal signals + updates + `wait_condition` — inside `DeliveryWorkflow`, with a domain-rich operator decision set that exceeds the canonical {approve, reject, edit, escalate} taxonomy documented across primary sources. The autonomous lane (gateway run loop for specialist chat, A2A threads, `sessions_spawn`, ACP) does **not** currently support async suspend/resume at the tool-call boundary: `before_tool_call` is a synchronous hook that returns `{blocked, reason}` in-process, with no primitive for awaiting an external human decision.

The verdict is therefore **feasible, lane-specific**. For the deterministic lane, generalising the `DeliveryWorkflow` template into a reusable "approval-sidecar workflow" primitive — owned by `bot-config-api` and invoked from any harness run that needs an approval gate — is a small, low-risk step that reuses what already works. For the autonomous lane, the cleanest v1 is to teach `before_tool_call` to **route sensitive tool calls to the same approval-sidecar workflow and block synchronously on the result with a bounded timeout**, falling back to a policy-defined default (deny or escalate) on timeout. This requires a real but scoped gateway change, not a rewrite.
...
### 4.3 A2A checkpoints are architected but implementation-shape needs verification

`docs/architecture/a2a-full-design.md` describes an explicit `PENDING_APPROVAL` thread state, auto-checkpoint after N turns, and a decision set of "approve (resume), reject (archive), or approve with guidance (inject new direction)". This is the right conceptual shape. The actual implementation — whether A2A threads persist their approval state durably across bot restarts — is not verified in this memo and is flagged in §13 as an open question.

### 4.4 `before_tool_call` hook is synchronous and does not await human decisions

`agentopia-core/src/agents/pi-tools.before-tool-call.ts` (post-hook section) returns synchronously:

```ts
if (hookResult?.block) {
  return { blocked: true, reason: hookResult.blockReason || "Tool call blocked by plugin hook" };
}
```

The hook context carries `sessionKey`, `sessionId`, `runId`, `toolCallId`, `executionClass`. It has the IDs that an approval sidecar would need. What it does **not** have is a primitive for "suspend this tool-call and await an external decision." This is the single most important feasibility constraint the ARB must accept: **autonomous-lane HITL at the tool-call boundary requires a modest but real gateway change.**
...
| Runtime tool-call interception | **Partial** — sync hook only | `pi-tools.before-tool-call.ts` lines 179-184 | 🟡 Supports sync deny; does not support async await-for-human |
...
| A2A thread checkpoint | **Architected, implementation unverified** | `a2a-full-design.md` describes PENDING_APPROVAL | 🟡 Docs describe the shape; implementation-level state verification pending |
...
1. **Gateway async-await primitive at the tool-call boundary.** The single biggest refactor. Today `before_tool_call` is sync-only; the hook runner must be allowed to perform a bounded `await` on a control-plane HTTP call. This changes the gateway's tool-dispatch shape incrementally but not fundamentally.
2. **A2A thread durability verification.** `a2a-full-design.md` describes PENDING_APPROVAL conceptually. Before wiring A2A checkpoints to the sidecar, an engineering spike should verify whether A2A state survives bot-pod restarts today, and if not, what it takes to persist thread state in Postgres in the same shape the sidecar workflow expects.
...
1. **A2A thread durability.** Verify whether A2A threads currently persist `PENDING_APPROVAL` state across bot-pod restarts, or whether a Postgres migration is required before wiring A2A into the sidecar.
