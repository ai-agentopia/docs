...
- Specific vendor selection outside two explicitly scoped work packages (trace backend — **backend decision resolved by [ADR-015](../adrs/015-h3-02-trace-backend-langfuse): Langfuse; production service design drafted in [h3-observability-production-design](../architecture/harness-control/h3-observability-production-design) (Draft — phase-α shape specified, production steady-state target specified with open gaps)**; ACP runtime hardening surface).
...
- **Deliverables.** OTEL + OpenInference instrumentation (`WP-H3-01` — protocol-side groundwork landed in [protocol#487](https://github.com/ai-agentopia/agentopia-protocol/pull/487); core-side pending); trace-backend **decision** — resolved: Langfuse selected per [ADR-015](../adrs/015-h3-02-trace-backend-langfuse). Production service design drafted in [h3-observability-production-design](../architecture/harness-control/h3-observability-production-design): **phase-α deployment shape specified and ready to implement; production steady-state target specified with open architecture gaps — document still Draft, not Accepted** (`WP-H3-02`); eval harness keyed off `RunContract` (`WP-H3-03`); checkpoint policy matrix (`WP-H3-04`); promote `before_tool_call` to a pluggable checkpoint interception with expanded result contract — `pending`/`edited`/`escalated`/`timeout` (`WP-H3-05`); ApprovalSidecarWorkflow generic primitive (`WP-H3-06`); unify workflow / work-item / thread approval semantics behind the policy matrix (`WP-H3-07`).
...
| WP-H3-02 | Trace-backend ADR + production design | `docs` | Self-hosted constraint from CLAUDE.md | **Backend decision resolved** ([ADR-015](../adrs/015-h3-02-trace-backend-langfuse): Langfuse). Production service design drafted at [h3-observability-production-design](../architecture/harness-control/h3-observability-production-design) — **Draft**: phase-α shape specified, production steady-state target specified with open architecture gaps (§14 and §16 of that doc); promotion to Accepted is gated on its §14.7 criteria. |
...
- **Trace backend lock-in.** Backend decision resolved via [ADR-015](../adrs/015-h3-02-trace-backend-langfuse): Langfuse (MIT core). Reversibility preserved via OTLP + OpenInference emission — swap remains a deployment change, not a code rewrite. Phase-α deployment shape and production steady-state target are both specified in [h3-observability-production-design](../architecture/harness-control/h3-observability-production-design); the document is still **Draft** with open architecture gaps (steady-state storage-class choice, Postgres/Redis/ClickHouse HA topologies, cluster expansion to ≥ 3 nodes) that block its promotion to Accepted.
...
1. ~~**Trace backend — Phoenix or Langfuse?**~~ **Backend decision resolved** 2026-04-23 via [ADR-015](../adrs/015-h3-02-trace-backend-langfuse): Langfuse (MIT core). Production service design drafted in [h3-observability-production-design](../architecture/harness-control/h3-observability-production-design); the document is **Draft** with open architecture gaps for the production steady state (its §14 and §16). Gate-criteria for promoting the design to Accepted live in that document's §14.7 and are not duplicated here.
