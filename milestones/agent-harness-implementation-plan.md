...
- **Trace backend lock-in.** Backend decision resolved via [ADR-015](../adrs/015-h3-02-trace-backend-langfuse): Langfuse (MIT core). Reversibility preserved via OTLP + OpenInference emission — swap remains a deployment change, not a code rewrite. Production service design at [h3-observability-production-design](../architecture/harness-control/h3-observability-production-design) is still **Draft**; production target is **single-node with restore-from-backup durability (HA explicitly deferred)**; promotion blocked on off-cluster backup target (#168) and operational hardening criteria (§14.5 of that doc).
