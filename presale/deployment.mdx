...
Agentopia ships with separate development and validation environments. Each environment runs as an independent Kubernetes namespace with its own services, storage, and LLM routing. Environments are isolated from each other and promoted independently via ArgoCD.

Environment access is controlled — contact your Agentopia administrator for access details.
